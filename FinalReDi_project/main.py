...
def del_bd_all(message, random_symbols):
    if message.text == "Главное меню":
        ...
        if message.text == random_symbols:
            try:
                curbd = FunkBd.cur_bd(message.chat.id)
                conn = sqlite3.connect(f'{curbd}.db')
                cur = conn.cursor()
                users_all = cur.execute("select userid from users")
                users_all = map(lambda x: x[0], list(users_all))
                conn.close()
                ...
                btn4 = types.KeyboardButton("Мои Базы")
                markup3.add(btn1, btn2, btn3, btn4)
                for user in users_all:
                    cur.execute(
                        f"delete from id_databases where user_id = '{user}' and name_base = '{curbd}'")
                    conn.commit()
                    botTimeWeb.send_message(
                        user, f"База даных {curbd} была удаленна вы возвращены в главное меню", reply_markup=markup3)
                cur.execute(
                    f"Update user_name set cur_bd = NULL where cur_bd = '{curbd}'")
                conn.commit()
                conn.close()
                os.remove(f'{curbd}.db')
                if os.path.exists(f'{curbd}cr'):
                    shutil.rmtree(f'{curbd}cr')
                if os.path.exists(f'{curbd}completed'):
                    shutil.rmtree(f'{curbd}completed')
                botTimeWeb.send_message(
                    message.chat.id, f"Ваша база данных була успешно удалена, вы возвращены в главное меню", reply_markup=markup3)
            except Exception as iter:
                # print(iter)
                msg = botTimeWeb.send_message(
                    message.chat.id, f"Что-то пошло не так,попробуйте снова.")
                botTimeWeb.register_next_step_handler(
                    msg, del_bd_all, random_symbols=random_symbols)
        else:
            random_symbols = generate_random_string(5)
            msg = botTimeWeb.send_message(
                message.chat.id, f"Код не верен,попробуйте снова. Пожалуйста введите следующие символы: {random_symbols}")
            botTimeWeb.register_next_step_handler(
                msg, del_bd_all, random_symbols=random_symbols)
    ...
    try:
        if message.document.file_name.split(".")[-1].lower() in ['mp4', 'avi', 'mkv', 'mov', 'wmv', 'flv', 'webm']:
            ...
            file_path = str(FunkBd.cur_bd(message.from_user.id)) + "completed" + \
                "\\" + f"{id_task}-"
            with open(file_path+message.document.file_name, 'wb') as new_file:
                new_file.write(downloaded_file)
            if message.document.mime_type.startswith('image'):
                file_name_meta = message.document.file_name.split(".")
                with open(file_path+"".join(file_name_meta[:-1])+"-meta.txt", 'w') as new_file:
                    new_file.write(
                        get_meta(os.getcwd()+"\\"+file_path+message.document.file_name))
            if message.document.file_name.endswith(('.heic', '.heif', '.HEIC', '.HEIF')):
                name = "".join(message.document.file_name.split(".")[:-1])
                if convert_to_jpg(file_path+message.document.file_name, file_path+name):
                    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
                    btn1 = types.KeyboardButton("Главное меню")
                    markup.add(btn1)
                    botTimeWeb.send_message(
                        message.chat.id, "Что-то пошло не так, пожалуйста вернитесь в главное меню и попробуйте снова", reply_markup=markup)
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            btn1 = types.KeyboardButton("Добавить файлы")
            btn2 = types.KeyboardButton("Выполнить задание")
            btn3 = types.KeyboardButton("Главное меню")
...
def func(message):
    ...
    if (message.text == "Создать Базу"):
        ...
        if FunkBd.cr_or_not(message.chat.id):
            ...
            markup.add(btn1)
            botTimeWeb.send_message(
                message.chat.id, "При удаление БД все данные о сотрудниках и о заданиях будут потерянны без возможности востановления", reply_markup=markup)
            markup = types.InlineKeyboardMarkup()
            btn1 = types.InlineKeyboardButton(
                text="❌Да❌", callback_data=f"del_bd")
            btn2 = types.InlineKeyboardButton(
                text="✅Нет✅", callback_data=f"main_menu")
            markup.add(btn1, btn2)
            botTimeWeb.send_message(
                message.chat.id, "Вы уверенны что хотите удалить вашу БД?", reply_markup=markup)
        else:
            botTimeWeb.send_message(
                message.chat.id, "У вас нет доступа к этой команде")
        ...
        if not FunkBd.cr_or_not(message.chat.id):
                ...
                for task in tasks:
                    ...
                    for c_t in tasks_c:
                        ...
                        for file in os.listdir(FunkBd.cur_bd(message.from_user.id)+"completed"):
                            file_s = file.split("-")
                            if file_s[0] == str(task[0]):
                                if file_s[-1] != "meta.txt":
                                    botTimeWeb.send_document(
                                        message.chat.id, open(FunkBd.cur_bd(message.from_user.id)+"completed"+'\\'+file, 'rb'))
                                if file_s[-1] == "meta.txt":
                                    with open(FunkBd.cur_bd(message.from_user.id)+"completed"+'\\'+file, "r") as f:
                                        botTimeWeb.send_message(
                                            message.chat.id, f"Информация о файле {''.join(file_s[1:-1])}:\n{f.read()}")
                    conn.close()
                    botTimeWeb.send_message(
                        message.chat.id, "🟰🟰🟰🟰🟰🟰🟰🟰🟰🟰🟰")
        else:
            botTimeWeb.send_message(
                message.chat.id, "У вас нет доступа к этой команде")
    elif (message.text == "Мои Базы"):
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        btn1 = types.KeyboardButton("Назад")
        markup.add(btn1)
        botTimeWeb.send_message(
            message.chat.id, "Выберете в какую базу данных вы хотите войти:", reply_markup=markup)
        conn = sqlite3.connect(f'userid_name.db')
        cur = conn.cursor()
        BDs = list(cur.execute(
            f"Select name_base from id_databases where user_id = '{message.chat.id}'"))
        if len(BDs) > 0:
            for DB in list(map(lambda x: x[0], BDs)):
                conn = sqlite3.connect(f'{DB}.db')
                cur = conn.cursor()
                emp = list(cur.execute(
                    f"Select * from users where userid = '{message.chat.id}'"))
                keyboard = types.InlineKeyboardMarkup()
                button1 = types.InlineKeyboardButton(
                    text="✅Войти в эту базу✅", callback_data=f"enter_in_bd:{DB}")
                keyboard.add(button1)
                botTimeWeb.send_message(
                    message.chat.id, f"Имя: {emp[0][1]}\nСтатус: {emp[0][2]}\nДолжность: {emp[0][3]}\nБаза:{DB}", reply_markup=keyboard)
                conn.close()
        else:
            botTimeWeb.send_message(
                message.chat.id, f"Вы еще не работали не с одной базой данных")
        conn.close()
...
def callback_query(call):
    ...
    if data[0] == "del_user":
        ...
        markup3.add(btn1, btn2, btn3, btn4)
        botTimeWeb.send_message(
            data[1], f"Вы были удаленны из базы даных {data[2]} и возвращены в главное меню", reply_markup=markup3)
        ...
        random_symbols = generate_random_string(5)
        msg = botTimeWeb.send_message(
            call.message.chat.id, f"Для подтверждения того что вы хотите удалить базу данных {FunkBd.cur_bd(call.message.chat.id)}, введите следующие символы: {random_symbols}")
        botTimeWeb.register_next_step_handler(
            call.message, del_bd_all, random_symbols=random_symbols)
        ...
        botTimeWeb.register_next_step_handler(
            msg, change_my_jt)
    elif data[0] == "enter_in_bd":
        Bd = FunkBd('userid_name')
        Bd.set_cur_bd(call.message.chat.id, str(data[1]))
        if FunkBd.cr_or_not(call.message.chat.id):
            markup = mar_creater()
            botTimeWeb.send_message(
                call.message.chat.id, text=f"Вы успешно вошли в базу данных {data[1]}, можете продолжать вашу роботу", reply_markup=markup)
        elif FunkBd.admin_or_not(call.message.chat.id, call.message.chat.id):
            markup = mar_admin()
            botTimeWeb.send_message(
                call.message.chat.id, text=f"Вы успешно вошли в базу данных {data[1]}, можете продолжать вашу роботу", reply_markup=markup)
